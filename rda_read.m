function [rda,rda_filename]=rda_read(varargin)
% INPUT: complete path to .rda file
% output: RDA structure
%
% Read spectroscopy data from Siemens machine
%
% Read a .rda file
%
% from: https://scion.duhs.duke.edu/svn/sample_data/siemens_rda_export/read_rda102.m
if nargin==0
...
bytes_per_point = 16;
rda.complex_data = fread(fid , rda.CSIMatrix_Size(1) * rda.CSIMatrix_Size(1) *rda.CSIMatrix_Size(1) *rda.VectorSize * 2 , 'double');  
...
% Now convert this data into something meaningful
 ...
 rda.fid = reshape(rda.complex_data,  2 , rda.VectorSize , rda.CSIMatrix_Size(1) ,  rda.CSIMatrix_Size(2) ,  rda.CSIMatrix_Size(3) );
 
 %Combine the real and imaginary into the complex matrix
 rda.fid = squeeze(complex(rda.fid(1,:,:,:,:),rda.fid(2,:,:,:,:)));
 
 %Do fft
 rda.freq = fftshift(fft(rda.fid,1024,1),1)

%  rda.data = fft(fft(fftshift(fftshift(padarray(fftshift(fftshift(ifft(ifft(rda.Time_domain_data,[],2),[],3),2),3),[0 16 16],'both'),2),3),[],2),[],3);
     end 
